...
export function format(v) {
  ...
  if (v.openAiModels?.length) {
    for (const m of v.openAiModels) {
      lines.push(`openAiModels(${m.service}): ${m.ok ? m.ids.join(", ") || "(empty)" : m.error}`);
    }
  }
...
export async function execute(args) {
  ...
  for (const s of openServices.filter((x) => x.id === "vllm" || x.id === "llama" || x.id === "lmstudio")) {
    ...
    if (models.ok && models.ids.length) {
      openAiNotes.push(
        `${s.id} /v1/models ok (${models.ids.slice(0, 5).join(", ")}). Use these ids in providerSnippets.`,
      );
    } else if (!models.ok) {
      openAiNotes.push(`${s.id} /v1/models failed (${models.error}) — port open but API not ready; also try docker_doctor.`);
    }
  }
